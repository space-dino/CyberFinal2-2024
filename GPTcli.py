...
class Client:
    ...
    def should_update_frame(self, index):
        # Only update frames if there's a change or at a set interval
        # To avoid unnecessary GUI updates
        return index < len(self.labels) and time.time() - self.last_update_times[index] > 0.05

    def update_frame_time(self, index):
        if len(self.last_update_times) <= index:
            self.last_update_times.append(time.time())
        else:
            self.last_update_times[index] = time.time()
    ...
    def draw_GUI_frame(self, frame, index, fps=None):
        if self.is_sharing_screen:
            ...
            self.root.deiconify()
            if self.should_update_frame(index):
